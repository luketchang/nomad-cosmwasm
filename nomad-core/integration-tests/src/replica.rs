...
use common::{
    addr_to_h256, h256_to_string, replica, test::test_replica, Encode, MessageStatus, NomadMessage,
};
...
fn processes_proved_message() {
    ...
    let res = app
        .execute_contract(sender.clone(), replica_addr.clone(), &prove_msg, &[])
        .unwrap();
...
fn proves_and_processes_message() {
    ...
    // Prove leaf under committed root
    let msg = test_replica::ExecuteMsg::ReplicaExecuteMsg(replica::ExecuteMsg::ProveAndProcess {
        message: nomad_message.to_vec(),
        proof: path,
        index: index.try_into().unwrap(),
    });
